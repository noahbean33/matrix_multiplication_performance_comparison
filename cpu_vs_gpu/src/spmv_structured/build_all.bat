@echo off
REM Convenience script to build all SpMV implementations on Windows
...
if %ERRORLEVEL% EQU 0 (
    ...
    if exist build rmdir /s /q build
    mkdir build
    g++ -O3 -std=c++17 -o build/host_sw.exe host.cpp spmv_kernel.cpp -I.
    cd ..
...
echo Build Summary
echo ========================================
echo CPU:  cpu\build\Release\spmv_cpu.exe
where nvcc >nul 2>nul
if %ERRORLEVEL% EQU 0 (
    echo GPU:  gpu\build\Release\spmv_gpu.exe
) else (
...
echo   cd cpu\build\Release ^&^& spmv_cpu.exe
echo   cd gpu\build\Release ^&^& spmv_gpu.exe
