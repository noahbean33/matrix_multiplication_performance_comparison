@echo off
REM Convenience script to build all Streaming Stencil implementations on Windows

echo ========================================
echo Building Streaming Stencil Benchmark
echo ========================================
...
if %ERRORLEVEL% EQU 0 (
    ...
    if exist build rmdir /s /q build
    mkdir build
    g++ -O3 -std=c++17 -o build/host_sw.exe host.cpp stencil_kernel.cpp -I.
    cd ..
...
echo Build Summary
echo ========================================
echo CPU:  cpu\build\Release\stencil_cpu.exe
where nvcc >nul 2>nul
if %ERRORLEVEL% EQU 0 (
    echo GPU:  gpu\build\Release\stencil_gpu.exe
) else (
...
echo   cd cpu\build\Release ^&^& stencil_cpu.exe
echo   cd gpu\build\Release ^&^& stencil_gpu.exe
echo   cd fpga\build ^&^& host_sw.exe
echo.
echo Key Insight:
echo   This is FPGA's HOME TURF!
echo   Line buffer streaming: 1 point per cycle
echo   FPGA shows its UPPER BOUND performance
echo.
pause
